...
def check_log_connections_setting(server_name, resource_group, subscription_id):
    """Checks if 'log_connections' is set to 'on' for a PostgreSQL Single Server."""
    command = f"az postgres server configuration show --resource-group {resource_group} --server-name {server_name} --subscription {subscription_id} --name log_connections -o json"
    result = run_command(command)
    
    if result:
        try:
            config = json.loads(result)
            log_connections = config.get("value", "off")
            print(f"{Fore.YELLOW}____________________________________________________{Style.RESET_ALL}")
            print()

            
            if log_connections == "on":
                print(f"PostgreSQL Server: {server_name} - log_connections: {Fore.GREEN}{log_connections}{Style.RESET_ALL}")
                return True
            else:
                print(f"PostgreSQL Server: {server_name} - log_connections: {Fore.RED}{log_connections}{Style.RESET_ALL}")
                return False
        except json.JSONDecodeError:
            print(f"{Fore.RED}Failed to parse log_connections setting for server {server_name}.{Style.RESET_ALL}")
            return False
    else:
        print(f"{Fore.RED}Failed to retrieve log_connections setting for server {server_name}.{Style.RESET_ALL}")
        return False

def main():
    """Main function to check the 'log_connections' setting for all PostgreSQL Single Servers across subscriptions."""
    ...
    for subscription in subscriptions:
        ...
        for server in servers:
            ...
            is_compliant = check_log_connections_setting(server_name, resource_group, subscription_id)
            if not is_compliant:
